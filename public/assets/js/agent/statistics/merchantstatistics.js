...
//defaultValue:this.today(0)+' 00:00:00 - '+this.today(0)+ ' 23:59:59' 
                        {field: 'pay_type', title: __('通道'),visible: false, placeholder: '选择通道',addclass: "selectpage",extend:'data-source="thirdacc.agentacc/getAccForSelect" data-primary-key="code" data-multiple="false" data-select-only="false" data-order-by="id" '},
                            ...
                            buttons: [
                                {
                                    ...
                                    //icon: 'fa fa-folder-o',
                                    url: 'statistics.merchantstatistics/useraccstatistics',
                                    // hidden:function(row){
                                    //     if(row.level != 1){
                                    //         return true;
                                    //     }
        ...
        useraccstatistics: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'statistics.merchantstatistics/useraccstatistics/ids/' + Config.ids + location.search,
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        {checkbox: true},
                        //{field: 'id', title: __('Id'), operate:false},
                        {field: 'pay_type', title: __('Acc_code'),operate:false},
                        {field: 'acc_name', title: __('Acc.name'), operate:false},
                        {field: 'time', title: __('时间'),visible: false,operate:false},
                        {field: 'createtime', title: __('时间'),visible: false, operate:'RANGE', addclass:'datetimerange', formatter: Table.api.formatter.datetime,defaultValue:this.today(0)+' 00:00:00 - '+this.today(0)+ ' 23:59:59' },
                        {field: 'amount', title: __('金额'), operate:false},
                        {field: 'count', title: __('笔数'), operate:false},
                        //{field: 'yesterday_suc_money', title: __('Yesterday_suc_money'), operate:false},
                        {field: 'fees', title: __('Rate'), operate:false},
                        {field: 'success_rate', title: __('成率'), operate:false},
                    ]
                ],
                search:false,
                showSearch: false,
            ...
            table.on('load-success.bs.table', function (e, data) {
                ...
                //这里我们手动设置底部的值
                $("#allmoney").text(data.extend.allmoney);
                $("#allnum").text(data.extend.allnum);
                $("#allbalance").text(data.extend.allbalance);
                $("#allfees").text(data.extend.allfees);
            });
        },
        api: {
