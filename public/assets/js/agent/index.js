define(['jquery', 'bootstrap', 'backend', 'addtabs', 'adminlte', 'form'], function ($, undefined, Backend, undefined, AdminLTE, Form) {
    var Controller = {
        index: function () {
            ...
            // 检查是否需要弹出绑定谷歌验证码的窗口
            if (Config.is_bind_google === 0) {
                $('#bindGoogleAuthModal').modal('show');
            ...
            $('#copyGoogleSecretBtn').click(function() {
                var googleSecret = $('#googleSecret').val();
                
                Toastr.error('复制失败，请扫码');
                ...
                /*navigator.clipboard.writeText(googleSecret).then(function() {
                    console.log('谷歌密钥已复制到剪贴板');
                }, function() {
                    console.error('无法复制谷歌密钥到剪贴板');
                });*/
            ...
            // 绑定谷歌验证码的提交事件
            $('#bindGoogleAuthBtn').click(function () {
                $.ajax({
                    url: "index/bindGoogleAuth",
                    type: 'post',
                    data: $('#bindGoogleAuthForm').serialize(),
                    dataType: 'json',
                    success: function (data) {
                        if (data.code === 1) {
                            $('#bindGoogleAuthModal').modal('hide');
