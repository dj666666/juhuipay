define(['jquery', 'bootstrap', 'backend', 'addtabs', 'table', 'form', 'echarts', 'echarts-theme', 'template'], function ($, undefined, Backend, Datatable, Table, Form, Echarts, undefined, Template) {
    ...
    var Controller = {
        ...
        index: function () {
            ...
            /*obj.chart3 = this.api.index.myChart3();
        ...
        api: {
            ...
            index: {
                    ...
                    function refresh() {
                        ...
                        si = setTimeout(function () {
                            Fast.api.ajax({
                                ...
                                option.series[1].data = res.orderSaleAmount;
                                myChart.clear();
                                myChart.setOption(option, true);
                                return false;
                            });
                ...
                /*myChart3: function () {
                    // 基于准备好的dom，初始化echarts实例
                    let myChart = Echarts.init($('#echarts3')[0], 'macarons');
                    // 指定图表的配置项和数据
                    let option = {
                        title: {
                            text: '订单分布',
                        ...
                        tooltip: {
                            ...
                            formatter: function (row) {
                                ...
                                let nums = (data && data.nums) || 0;
                                let rate = (data && data.rate) || 0;
                                return name + ` : ( ${rate}% )<br/>订单数量 : ` + value + '个<br/>订单金额 : ' + amount + '元<br/>商品数量 : ' + nums + '件';
                            }
                        },
                        legend: {
                            orient: 'vertical',
                        ...
                        series: [{
                            ...
                            data: Config.mapOrderList
                        }]
                    }

                    Config.mapOrderList.sort((a, b) => {
                        return b.value - a.value;
                    })
                    option.visualMap.max = Config.mapOrderList[0].value == 0 ? 5 : Config.mapOrderList[0].value;
                    myChart.setOption(option, true);
                    ...
                    myChart.on('click', function ({
                       ...
                       if(res.id){
                            Fast.api.open('shop/report/areas?id=' + res.id, name, {
                                area: ["90%", "90%"]
                            })
                       }
                    });
                    ...
                    function refresh() {
                        ...
                        si = setTimeout(function () {
                            Fast.api.ajax({
                                ...
                                option.series[0].data = res.mapOrderList;
                                res.mapOrderList.sort((a, b) => {
                                    return b.value - a.value;
                                })
                                option.visualMap.max = res.mapOrderList[0].value == 0 ? 5 : res.mapOrderList[0].value;
                                myChart.clear();
                                myChart.setOption(option, true);
                                return false;
                            });
