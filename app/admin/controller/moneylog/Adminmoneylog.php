...
class Adminmoneylog extends Backend
{
    ...
    /**
     * Adminmoneylog模型对象
     * @var \app\admin\model\moneylog\Adminmoneylog
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\moneylog\Adminmoneylog;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            foreach ($list as $row) {
                $row->visible(['id','out_trade_no','type','fees','amount','before_amount','after_amount','create_time','remark','ip_address','is_automatic']);
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    $merchant = Db::name('admin')->where('id',$this->auth->id)->find();
                    ...
                    switch ($params['type']) {
                        case 0:
                            ...
                            Db::name('admin')->where(['id'=>$merchant['id'],'last_money_time'=>$merchant['last_money_time']])->update(['money'=>$new_money,'last_money_time'=>time()]);
