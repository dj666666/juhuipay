...
class Useracc extends Backend
{
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    $findAcc = Db::name('user_acc')->where(['user_id' => $params['user_id'], 'acc_code' => $params['acc_code'] ]);
                    if($findAcc){
                        $this->error('该码商已经添加过'.$params['acc_code'].'通道');
                    }
                    
                    $checkRes = Utils::checkParentRate($params['user_id'], $params['acc_code'], $params['rate'], 'add');
                    if ($checkRes){
    ...
    public function syncUserAcc() {
        ...
        $userList = Db::name('user')->where('status','normal')->field('id,agent_id')->select();
        if($userList){
