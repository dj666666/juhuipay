 ...
 * @icon fa fa-circle-o
 */
class Roborder extends UserBackend
{
    ...
    public function checkUser() {
        $user = Db::name('user')->where(['id' => $this->auth->id, 'status' => 'normal'])->find();
        if (!$user) {
    ...
    public function index() {
        ...
        if ($this->request->isAjax()) {
            ...
            $total = $this->model
                //->withJoin(['merchant'])
                ->where(['agent_id' => $this->auth->agent_id, 'status' => Dforder::ORDER_STATUS_DEALING, 'is_robbed' => 0, 'is_lock'=>'0'])
                ->where($where)
                ->order($sort, $order)
                ->count();
            ...
            $list = $this->model
                //->withJoin(['merchant'])
                ->where(['agent_id' => $this->auth->agent_id, 'status' => Dforder::ORDER_STATUS_DEALING, 'is_robbed' => 0, 'is_lock'=>'0'])//is_third_df
                ->where($where)
                ->order($sort, $order)
                ->limit($offset, $limit)
                ->select();
    ...
    public function robOrder($ids = null) {
        ...
        if ($count >= Config::get('site.rob_order_num')) {
            $this->error('先处理订单');
        }
