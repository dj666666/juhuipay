...
class Auth extends \fast\UserAuth
{
    protected $_error = '';
    ...
    public function login($username, $password, $googlecode, $keeptime = 0)
    {
        ...
        //判断是否开启谷歌验证
        if(Config::get('site.login_checkusergoogle') == 1 and !empty($admin->google_code)){
            if(!$googlecode){
                $admin->loginfailure++;
                $admin->save();
        ...
        //判断是否限制ip登入
        if (Config::get('site.loginip_checkuserlogin')) {
    ...
    public function isLogin()
    {
        ...
        if (Config::get('site.login_unique_user') == '1') {
        ...
        //判断管理员IP是否变动
        if (Config::get('site.loginip_user_check') == '1') {
    ...
    public function getChildrenGroupIds($withself = false)
    {
        ...
        // 取出所有分组
        $groupList = \app\user\model\AuthGroup::where(['status' => 'normal'])->select();
        $objList = [];
        foreach ($groups as $k => $v) {
            if ($v['rules'] === '*') {
                $objList = $groupList;
    ...
    public function getChildrenAdminIds($withself = false)
    {
        ...
        if (! $this->isSuperAdmin()) {
            ...
            $authGroupList = \app\user\model\AuthGroupAccess::
            field('uid,group_id')
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        // 必须将结果集转换为数组
        $ruleList = \app\user\model\AuthRule::where('status', 'normal')
            ->where('ismenu', 1)
            ->order('weigh', 'desc')
            ->cache('__user_menu__')
            ->select()->toArray();
        $indexRuleList = \app\user\model\AuthRule::where('status', 'normal')
            ->where('ismenu', 0)
