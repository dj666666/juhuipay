...
namespace app\merchant\controller\statistics;
 ...
 * @icon fa fa-circle-o
 */
class Merchantstatistics extends MerchantBackend
{
    
    /**
     * Merchantstatistics模型对象
     * @var \app\merchant\model\statistics\Merchantstatistics
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\merchant\model\statistics\Merchantstatistics;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            foreach ($list as $row) {
                ...
                //该用户总金额
                $row['all_money'] = Db::name('order')->where(['mer_id'=>$row['id'],'status'=>1])->where($map)->sum('amount');
                //该用户成功总金额
                $row['all_success_money'] = Db::name('order')->where(['mer_id'=>$row['id'],'status'=>1])->where($map)->sum('amount');
                //该用户失败总金额
                $row['all_fail_money'] = Db::name('order')->where(['mer_id'=>$row['id']])->where('status',3)->where($map)->sum('amount');

                //该用户总订单数量
                $row['all_order'] = Db::name('order')->where(['mer_id'=>$row['id']])->where($map)->count();
                //该用户总成功订单数量
                $row['all_success_order'] = Db::name('order')->where(['mer_id'=>$row['id'],'status'=>1])->where($map)->count();
                //该用户总失败订单数量
                $row['all_fail_order'] = Db::name('order')->where(['mer_id'=>$row['id']])->where('status',3)->where($map)->count();
            ...
            $allfailemoney = Db::name('order')->where('status',3)->where($map)->sum('amount');
            //总订单数量
            $allorder = Db::name('order')->where($map)->count();
            //总成功数量
            $allsuccessorder = Db::name('order')->where(['status'=>1])->where($map)->count();
            //总异常数量
            $allfaileorder = Db::name('order')->where('status',3)->count();
            //总手续费
