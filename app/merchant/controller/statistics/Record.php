 ...
 */
class Record extends MerchantBackend
{
    ...
    protected function getIncomeStatisticsData($date = '')
    {
        if ($date) {
            ...
            $starttime = \fast\Date::unixtime('month', 0, 'begin');
            $endtime = \fast\Date::unixtime('month', 0, 'end');
        }
        ...
        // 订单金额
        $orderAmountList = \app\merchant\model\order\Order::where('createtime', 'between time', [$starttime, $endtime])->where('status',1)
            ->field('createtime, SUM(amount) AS amount, DATE_FORMAT(FROM_UNIXTIME(createtime), "' . $format . '") AS pay_date')
            ->group('pay_date')
            ->select();
            
        //订单手续费
        $orderFeesList = \app\merchant\model\order\Order::where('createtime', 'between time', [$starttime, $endtime])->where('status',1)
            ->field('createtime, SUM(fees) AS amount, DATE_FORMAT(FROM_UNIXTIME(createtime), "' . $format . '") AS pay_date')
            ->group('pay_date')
            ->select();

        $applyFeesList = \app\merchant\model\yhk\Apply::where('createtime', 'between time', [$starttime, $endtime])->where('status',1)
            ->field('createtime, SUM(fees) AS amount, DATE_FORMAT(FROM_UNIXTIME(createtime), "' . $format . '") AS pay_date')
            ->group('pay_date')
            ->select();
