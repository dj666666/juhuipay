...
class Auth extends \fast\MerchantAuth
{
    protected $_error = '';
    ...
    {
        return Session::get('merchant.'.$name);
    }
    ...
    public function login($username, $password, $googlecode, $keeptime = 0)
    {
        ...
        //判断是否限制ip登入
        if(Config::get('site.loginip_checkmerchantlogin')){
            $login_ip = explode(",",$admin['login_ip']);
        ...
        if (Config::get('fastadmin.login_failure_retry') && $admin->loginfailure >= 5 && time() - $admin->updatetime < 86400) {
    ...
    public function isLogin()
    {
        ...
        $admin = Session::get('merchant');
        if (! $admin) {
            return false;
        }
        //判断是否同一时间同一账号只能在一个地方登录
        if (Config::get('site.login_unique_merchant') == '1') {
        ...
        //判断管理员IP是否变动
        if (Config::get('site.loginip_merchant_check') == '1') {
    ...
    public function getChildrenGroupIds($withself = false)
    {
        ...
        // 取出所有分组
        $groupList = \app\merchant\model\AuthGroup::where(['status' => 'normal'])->select();
        $objList = [];
        foreach ($groups as $k => $v) {
            if ($v['rules'] === '*') {
                $objList = $groupList;
    ...
    public function getChildrenAdminIds($withself = false)
    {
        ...
        if (! $this->isSuperAdmin()) {
            ...
            $authGroupList = \app\merchant\model\AuthGroupAccess::
            field('uid,group_id')
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        // 必须将结果集转换为数组
        $ruleList = \app\merchant\model\AuthRule::where('status', 'normal')
            ->where('ismenu', 1)
            ->order('weigh', 'desc')
            ->cache('__merchant_menu__')
            ->select()->toArray();
        $indexRuleList = \app\merchant\model\AuthRule::where('status', 'normal')
            ->where('ismenu', 0)
