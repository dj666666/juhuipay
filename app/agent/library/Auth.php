...
class Auth extends \fast\AgentAuth
{
    protected $_error = '';
    ...
    public function isLogin()
    {
        ...
        if (Config::get('site.login_unique_agent') == '1') {
        ...
        //判断管理员IP是否变动
        if (Config::get('site.loginip_agent_check') == '1') {
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        // 必须将结果集转换为数组
        $ruleList = \app\agent\model\AuthRule::where('status', 'normal')
            ->where('ismenu', 1)
            ->order('weigh', 'desc')
            ->cache('__agent_menu__')
            ->select()->toArray();
