...
class Index extends AgentBackend
{
    ...
    public function index()
    {
        ...
        $user = Db::name('agent')->where('id', $this->auth->id)->find();
    ...
    public function bindGoogleAuth(){
        ...
        if ($this->request->isPost()) {
            ...
            $user = Db::name('agent')->where('id', $this->auth->id)->update(['google_code' => $googleSecret]);
            $this->success('绑定成功');
        }
    ...
    public function login()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $result = $this->auth->login($username, $password, $googlecode,$keeplogin ? 86400 : 0);
            if ($result === true) {
