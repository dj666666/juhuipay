 ...
 */
class Record extends AgentBackend
{
    ...
    protected function getIncomeStatisticsData($date = '')
    {
        ...
        // 订单金额
        $orderAmountList = \app\agent\model\order\Order::where('createtime', 'between time', [$starttime, $endtime])->where('status',1)
            ->field('createtime, SUM(amount) AS amount, DATE_FORMAT(FROM_UNIXTIME(createtime), "' . $format . '") AS pay_date')
            ->group('pay_date')
            ->select();
            
        //订单手续费
        $orderFeesList = \app\agent\model\order\Order::where('createtime', 'between time', [$starttime, $endtime])->where('status',1)
            ->field('createtime, SUM(fees) AS amount, DATE_FORMAT(FROM_UNIXTIME(createtime), "' . $format . '") AS pay_date')
            ->group('pay_date')
            ->select();

        $applyFeesList = \app\agent\model\yhk\Apply::where('createtime', 'between time', [$starttime, $endtime])->where('status',1)
            ->field('createtime, SUM(fees) AS amount, DATE_FORMAT(FROM_UNIXTIME(createtime), "' . $format . '") AS pay_date')
            ->group('pay_date')
            ->select();
