...
class Group extends AgentBackend
{
    ...
    public function index()
    {
        if ($this->request->isAjax()) {
            $list = AuthGroup::where('agent_id',$this->auth->id)->select(array_keys($this->groupdata));
            $list = $list->toArray();
            $groupList = [];
            foreach ($list as $k => $v) {
                $groupList[$v['id']] = $v;
    ...
    public function roletree()
    {
        ...
        if (($pid || $parentGroupModel) && (! $id || $currentGroupModel)) {
            $id = $id ? $id : null;
            $ruleList = AuthRule::where('status','normal')->order('weigh', 'desc')->order('id', 'asc')->select()->toArray();
            //读取父类角色所有节点列表
            $parentRuleList = [];
